...
void Wishbone::write(int width, uint64_t addr, uint64_t value)
{
   // if(width < granularity ) {
        //char msg[64] = "Unexpected write width %d"; // we sprintf to self, because width is never longer than 2 digits
    ...
    *wb_addr = (uint8_t)((addr / granularity) & ((1 << addr_lines) - 1));   //fixed
    *wb_wr_dat = value;
...
{
    //if(width < granularity) {
       // char msg[64] = "Unexpected read width %d"; // we sprintf to self, because width is never longer than 2 digits
       // sprintf(msg, msg, width);
       // throw msg;
   // }
    *wb_we = 0;
